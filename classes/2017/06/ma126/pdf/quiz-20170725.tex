...
\newcommand{\headerText}{
  MA 126-103 | Summer 2017 | Dr. Clontz | 2017 July 25 Quiz
}
...
\begin{tikzpicture}[x=1in,y=1in]
  ...
  \node[anchor=west] at (0.1,0.17) {Date: \textbf{2017 July 25}};
...
Use the fact that \(\frac{1}{1-x}=\sum_{k=0}^\infty x^k\) for
\(0<x<1\) to find a power series converging to \(f(x)=\frac{1}{1-2x+x^2}\)
for \(0<x<1\). (Hint: What's the derivative of \(\frac{1}{1-x}\)?)



\exerciseHeader{2017 July 24}{C16: Approx.}{
...
\(|R_n(1/2)|<0.01\). Then, approximate the value of \(\sqrt{e}\)
with an error no larger than \(0.01\).
