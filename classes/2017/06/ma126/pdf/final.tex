...
\newcommand{\headerText}{
  MA 126-103 | Summer 2017 | Dr. Clontz | Final Exam
}
...
\begin{tikzpicture}[x=1in,y=1in]
  ...
  \node[anchor=west] at (0.1,0.17) {Date: \textbf{2017 July 26}};

  \node[anchor=west] at (4.6,0.83) {Exercise Type (Cost):};
  \node[anchor=west] at (4.7,0.5) {\textbf{Final Exam (0AP each)}};
\end{tikzpicture}
...
\exerciseHeader{2017 July 21}{C15a: TaySer.}{
Generate a Taylor or Maclaurin series from a function.
}{1/3}
...
\exerciseHeader{2017 July 21}{C15b: TaySer.}{
Generate a Taylor or Maclaurin series from a function.
}{1/3}
...
Use the fact that \(\sinh(x)=\sum_{k=0}^\infty\frac{x^{2k+1}}{(2k+1)!}\) for
all real numbers \(x\) to find a power series converging to \(f(x)=\sinh(x^2)\)
for all real numbers \(x\).



\exerciseHeader{2017 July 24}{C16: Approx. (worth double)}{
Use Taylor’s formula to approximate values within appropriate margins of error.
}{1/2}

The Maclaurin Series for \(\cosh(x)\) is given by
\(\sum_{k=0}^\infty\frac{x^{2k}}{(2k)!}=1+\frac{x^2}{2!}+\frac{x^4}{4!}+\dots\),
and the Taylor polynomial error at \(x\) for \(a=0\) is given by
\(|R_n(x)|=\frac{|f^{(n+1)}(x_n)|}{(n+1)!}|x|^{n+1}\) for some value of
\(x_n\) between \(0\) and \(x\).
First, find a sufficiently large value of \(n\) such that
\(|R_n(1)|<0.01\), given that \(\sinh(1)\leq\cosh(1)\leq e\leq 3\). Then,
approximate the value of \(\cosh(1)\) with an error no larger than \(0.01\).
