...
\newcommand{\headerText}{
  MA 227-103 | Summer 2017 | Dr. Clontz | FINAL EXAM (example)
}
...
\begin{itemize}
  ...
  \item You have \textbf{120 minutes} to complete up to \textbf{18 exercises}
        of the 36 exercises provided in a separate book:
  ...
  \item Up to three \(\star\) marks will be converted to \checkmark{} marks.
        Students receiving less \(\times\) marks for quizzes given
        on or after July 06 compared to the rest of the class will have
        one or two additional \(\star\) marks converted to \checkmark{} marks.
...
%
%
% \exerciseHeader{2017 June 28}{C07: DoubleInt.}{
...
% }{a}
%
% Sketch the surface \((x+1)^2+(y-3)^2+z^2=1\) in \(xyz\) space.
%
% \vfill
%
% \exerciseHeader{2017 June 12}{C01: SurfaceEQ.}{
...
% }{b}
%
% Sketch the equation \(yz=9\) first as a curve in the \(yz\) plane,
% then as a surface in \(xyz\) space.
%
% \vfill\newpage
%
...
%
% Give a vector function modeling the line passing through
% \(\<3,3,2\>\) and parallel to the vector \(\<3,-1,4\>\).
%
% \vfill
%
% \exerciseHeader{2017 June 13}{C02: VectFunc.}{
...
% }{b}
%
% Give a vector function \(\vect r(t)\) parameterizing one counter-clockwise
% motion around the circle with center
% \(\<5,0\>\) from the point \(\<5,3\>\) back to itself.
%
% \vfill\newpage
%
% \exerciseHeader{2017 June 12}{C03: VectCalc.}{
...
%
% Find a vector tangent to the curve parameterized by
% \(\vect r(t)=\<3t,2t^2+t-2\>\) at the point \(\<0,-2\>\).
%
% \vfill
%
% \exerciseHeader{2017 June 14}{C03: VectCalc.}{
...
% }{b}
%
% Find \(\vect r(t)\) given \(\vect r'(t)=\<e^t,3t^2\>\) and
% \(\vect r(1)=\<2e,3\>\).
%
% \vfill\newpage
%
...
%
% Find the arclength parameter \(s(t)\) for the curve given by
% \(\vect r(t)=\<4\cos t,3t,-4\sin t\>\). Then give the
% arclength from \(t=0\) to \(t=\pi\).
%
% \vfill
%
% \exerciseHeader{2017 June 19}{C04: VectFuncSTNB.}{
...
% }{b}
%
% Suppose the unit tangent and normal vectors at a point on
% a parametrized curve are given
% by \(\vect T=\<\frac{3}{5},0,-\frac{4}{5}\>\) and
% \(\vect N=\<\frac{2}{5},\frac{\sqrt 3}{2},\frac{3}{10}\>\).
% Find the binormal vector \(\vect B\) at that same point.
%
% \vfill\newpage
%
% \exerciseHeader{2017 June 20}{C05: MulivarCalc.}{
...
% }{a}
%
% Find the minimal value of the directional derivative for
% the function \(f(x,y)=4xy+2x^2+y-1\) at the
% point \(\<-1,2\>\).
%
% \vfill
%
...
% }{b}
%
% Verify the mixed derivative theorem \(f_{yz}=f_{zy}\) for
% \(f(x,y,z)=xy^3-5xyz^2\) by computing the second partial derivative
% both ways.
%
% \vfill\newpage
%
...
% }{a}
%
% Use the multivariable Chain Rule to find \(\frac{df}{dt}\) at \(t=1\) given
% \(f(x,y)=3xy^2\) and \(\vect r(t)=\<t^2+2,1-2t\>\).
%
% \vfill
%
% \exerciseHeader{2017 June 22}{C06: ChainRule.}{
...
% }{b}
%
% Find an equation for the plane tangent to the surface \(xy=z^2+3\)
% at the point \(\<2,2,-1\>\).
%
% \vfill\newpage
%
...
%
% In \(xy\) plane,
% sketch the vector \(\vect v=\<3,4\>\), the vector \(\vect w\) pointing
% from \(\<3,4\>\) to \(\<-1,2\>\), and the vector \(\vect v+\vect w\).
%
% \vfill
%
%
...
% }{}
%
% Find the work done by a force vector \(\<5,-3\>\) over
% the displacement vector \(\<3,-4\>\).
%
% \vfill\newpage
%
% \exerciseHeader{2017 June 06}{S03: CrossProd.}{
...
% prove that \(\<3,1,-4\>\) and \(\<6,2,-8\>\) are parallel vectors.
%
% \vfill
%
%
...
%
% Assume \(g=10\) meters per second squared.
% Find the height of a projectile after \(3\) seconds if it is launched
% from the ground
% with initial velocity \(\<8,20\>\) meters per second.
%
% \vfill\newpage
%
% \exerciseHeader{2017 June 19}{S05: MulivarFunc.}{
...
% }{}
%
% Find the linearization \(L(x,y)\) for \(f(x,y)=(6x+3y)^{1/2}\) at the
% point \(\<1,1\>\). Then use it to show that
% \(f(1.1,0.98)\approx3.09\).
%
% \vfill\newpage
%
%
...
% }{}
%
% Find the maximum value of the function \(f(x,y)=2x^2+y^2+1\) on
% the closed and bounded disk \(x^2+y^2\leq 4\). (Hint:
% You can check the critical
% points on the boundary by using \(\vect{r}(t)=\<2\cos t,2\sin t\>\) with
% \(2\sin t\cos t=\sin(2t)\), or alternatively
% by using Lagrange multipliers with \(g(x,y)=x^2+y^2=4\).)
%
% \newpage
%
% \textcolor{gray}{Use this page if additional space is required.}
%
% \newpage
%
% \textcolor{gray}{Use this page if additional space is required.}
