...
% \exerciseHeader{2017 July 18}{C11: LineInt.}{
% Compute and apply line integrals.
% }{1/4}
%
% Find the work done by the force vector field \(\vect F=\<xy,2y\>\) along the
% line segment beginning at \(\<0,1\>\) and ending at \(\<3,4\>\).

% \exerciseHeader{2017 July 19}{C11: LineInt.}{
% Compute and apply line integrals.
% }{2/4}
%
% Rewrite \(\int_C (x+2y)\,ds\) as a definite integral with respect to \(t\),
...
% \exerciseHeader{2017 July 19}{C12: FundThmLine.}{
% Apply the Fundamental Theorem of Line Integrals.
% }{1/4}
%
% Find \(\int_C\vect F\cdot d\vect r\) where \(\vect F=\<2xz,-3z,x^2-3y\>\)
% and \(C\) is an unknown curve that begins at \(\<1,1,1\>\) and ends at
% \(\<2,0,1\>\).
...
% \exerciseHeader{2017 July 21}{C12: FundThmLine.}{
% Apply the Fundamental Theorem of Line Integrals.
% }{3/4}
%
% Compute the work done by the force vector field \(\<2xyz+3z^2,x^2z,6xz+x^2y\>\)
...
% }{3/3}
%
% Parameterize the plane \(x+2y+3z=6\).
...
% }{2/3}
%
% The function \(\vect{r}(\theta,z)=\<\cos\theta,\sin\theta,z\>\) parametrizes the
% cylinder \(x^2+y^2=1\).  Let \(\vect F=\<y,z,x\>\)
% and let \(S\) be the portion of the cylinder \(x^2+y^2=1\) where
% \(0\leq z\leq 2\) and \(y\geq 0\).
% Express the flux
% \(\iint_S \vect F\cdot\vect{n}\,d\sigma\) as a double iterated integral
...
Let \(C\) be the boundary of the unit square where \(0\leq x\leq 1\) and
\(0\leq y\leq 1\)
oriented counter-clockwise. Express the circulation of the vector field
\(\<3y^2,4x+3y\>\) around \(C\) as a double iterated integral.
(Do not solve this integral.)
...
Let \(D\) be the cube where \(1\leq x\leq 4\),  \(1\leq y\leq 4\),
and \(1\leq z\leq 4\).
Express the flux
\(\iint_{\partial D} \<xz,4xy^2,3xyz\>\cdot \vect{n}\,d\sigma\) as a triple
